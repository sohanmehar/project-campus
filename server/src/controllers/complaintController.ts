...
// @desc    Get all grievances/complaints across campus (Admin Oversight)
// @route   GET /api/v1/complaints/admin/all
...
export const getAllComplaints = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(500).json({ message: 'Error fetching all grievances', error: error.message });
  }
};

// @desc    Update grievance status / assign / resolution notes
...
export const updateComplaintStatus = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(500).json({ message: 'Error updating grievance ticket', error: error.message });
  }
};